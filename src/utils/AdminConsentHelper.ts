    ...
    public static async listenForAdminConsent(clientId: string, tenantId: string): Promise<boolean> {
        return new Promise<boolean>((resolve, reject) => {
            ...
            const server = http.createServer(async (req, res) => {
                ...
                if (!authError && !authErrorDescription) {
                    ...
                    if (!resolved) {
                        resolved = true;
                        server.close(() => {
                            reject(new Error(`Consent failed: ${authError} - ${authErrorDescription}`));
                        });
                    }
                }
            });
            ...
            server.listen(0, async () => {
                ...
                console.log('[AdminConsentHelper] Starting consent flow with redirect URI:', redirectUri);
                console.log('[AdminConsentHelper] Local server listening on port:', port);

                // Small delay to ensure server is fully ready (prevents race condition)
