 ...
 */
export async function createContainerTypeInput(context: ExtensionContext) {
	...
	const title = 'Create Container Type';

	async function inputAppName(input: MultiStepInput, state: Partial<State>) {
		state.containerTypeName = await input.showInputBox({
			title,
			step: 1,
			totalSteps: 1,
			value: '',
